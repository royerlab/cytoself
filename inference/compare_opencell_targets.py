...
protein_filter = dict(
	GLT8D1=[1,2,3,11,13,18,52,64,96,97]
	)
...
def get_nearest_proteins(
	...
	embeds_consensus_inf, labels_consensus_inf = get_consensus_encoding(
		embed_inf, protein_label_inf, sort_key=sort_key)
	...
	fname_stem = f"mode_{mode}__representation_{representation}__pcadim_{pca_dim}"
...
def get_embeds_and_crops(
	...
	if dataset == "opencell":
		...
		dir_embeddings = Path(
			"inference/results/get_crop_features") / dir_pretrained_model / f"ckpt_{checkpoint}"
		if representation == 'vqvec2':
			f_embed = dir_embeddings / "embeddings_vqvec2_opencell.pt"
		elif representation == 'hist':
			f_embed = dir_embeddings / "embeddings_vqindhist1_opencell.pt"
		else:
			raise
		...
		## these two lines are the old version where we didn't define the checkpoint
		# f_embed = Path(dir_pretrained_model) / "embeddings/vqvec2.npy"
	...
	## get inference embeddings for the new dataset
		...
		if representation == 'vqvec2':
			f_embed = dir_embeddings / "embeddings_vqvec2.pt"
		elif representation == 'hist':
			f_embed = dir_embeddings / "embeddings_vqindhist1.pt"
		else:
			raise
		...
		return embed_inf, labels_inf, crops_inf, df_meta_inf, df_annotations, df_inf_lookup
...
def find_nearest_analysis(embeds_consensus_opencell,
	...
	df_results_summary = pd.concat([
		df_results_summary, df_prot_nearest, df_dist_nearest,
...
def generate_inference_crop_grids(dir_pretrained_model, fname_crops, 
		nsamples=100, ncols=10):
	"""
	"""
	dir_results = Path(
		"inference/results") / current_filename.stem / dir_pretrained_model
...
if __name__ == "__main__":
	...
	# generate_inference_crop_grids("results/20231216_train_all_balanced_classes_2", fname_crops, fname_crops_meta)
	...
	dir_pretrained_models_checkpoints = [
		...
		# ["results/20231215_train_all_no_nucdist_balanced_classes_1.py", 14],
	]
	...
	for representation in ('vqvec2', 'hist'):
		for pca_dim in (None,200):
			for mode in ('correlation','euclidean'):
	...
	# 			'correlation',
	# 	]:
	# 		for do_umap in [False, True]:
	# 			res = analysis_choose_opencell_threshold(dir_pretrained_model,
	# 													 mode=mode,
	# 													 do_umap=do_umap)
	# 	ipdb.set_trace()
